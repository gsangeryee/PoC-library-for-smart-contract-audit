...
contract L1BossBridgeTest is Test {
    ...
    function testCanTransferFromValutToVault() public {
        address attacker = makeAddr("attacker");

        uint256 vaultBalance = 500 ether;
        deal(address(token), address(vault), vaultBalance);

        // Can trigger the deposit event, send transfer tokens to the vault
        vm.expectEmit(address(tokenBridge));
    ...
    function testSignatureReplay() public {
        ...
        // An attacker deposite token to L2
        vm.startPrank(attacker);
        token.approve(address(tokenBridge),type(uint256).max);
        ...
        // Signer / Operator is going to signt her withdrawal
